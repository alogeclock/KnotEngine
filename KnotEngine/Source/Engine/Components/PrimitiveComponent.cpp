...
void UPrimitiveComponent::Render(float DeltaTime, URenderer& Renderer)
{
    ...
    const FMatrix View = FMatrix::MakeViewLookAtLH(FVector(-5.0f, 0.0f, 0.0f), FVector::ZeroVector, FVector::UpVector);
    const FMatrix Projection = FMatrix::MakePerspectiveFovLH(KMath::ToRadian(60.0f), AspectRatio, 0.1f, 100.0f);
