...
void FD3D11Pipeline::Create(ID3D11Device* Device)
{
	...
	HRESULT Result = Device->CreateDepthStencilState(
	    &DepthStateDesc, DepthStencilState.ReleaseAndGetAddressOf());
	panicf(SUCCEEDED(Result), "ID3D11Device::CreateDepthStencilState 실패. HRESULT=0x{:08X}",
	...
	panicf(SUCCEEDED(Result), "ID3D11Device::CreateRasterizerState 실패. HRESULT=0x{:08X}",
	       static_cast<uint32>(Result));
	...
	panicf(SUCCEEDED(Result), "Vertex Shader 컴파일 실패. HRESULT=0x{:08X}\n{}",
	       static_cast<uint32>(Result), GetShaderErrorText(ErrorBlob.Get()));

	Result = Device->CreateVertexShader(
	    VertexShaderCode->GetBufferPointer(), VertexShaderCode->GetBufferSize(), nullptr,
	    VertexShader.ReleaseAndGetAddressOf());
	panicf(SUCCEEDED(Result), "ID3D11Device::CreateVertexShader 실패. HRESULT=0x{:08X}",
	       static_cast<uint32>(Result));
	...
	panicf(SUCCEEDED(Result), "ID3D11Device::CreatePixelShader 실패. HRESULT=0x{:08X}",
	       static_cast<uint32>(Result));
	...
	Result = Device->CreateBuffer(
	    &ConstantDesc, nullptr, ConstantBuffer.ReleaseAndGetAddressOf());
	panicf(SUCCEEDED(Result), "ID3D11Device::CreateBuffer(Constant) 실패. HRESULT=0x{:08X}",
	       static_cast<uint32>(Result));
}
...
void FD3D11Pipeline::UpdateConstant(
    ID3D11DeviceContext* DeviceContext, const FMatrix& WorldViewProjection)
{
	panic(DeviceContext);
	panic(ConstantBuffer);
...
ID3D11InputLayout* FD3D11Pipeline::GetOrCreateInputLayout(
    ID3D11Device* Device, const FVertexLayout& VertexLayout)
{
	if (InputLayout)
	{
		panicf(InputLayoutDescription == VertexLayout,
		       "Pipeline은 서로 다른 FVertexLayout을 동시에 지원하지 않는다.");
