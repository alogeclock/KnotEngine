    ...
    ID3D11Device* Device, Microsoft::WRL::ComPtr<IDXGISwapChain>&& InSwapChain)
{
	...
	Result = Device->CreateRenderTargetView(
	    FrameBuffer.Get(), nullptr, FrameBufferRTV.ReleaseAndGetAddressOf());
	...
	Result = Device->CreateTexture2D(
	    &DepthDesc, nullptr, DepthStencilBuffer.ReleaseAndGetAddressOf());
	panicf(SUCCEEDED(Result), "ID3D11Device::CreateTexture2D(DepthStencil) 실패. HRESULT=0x{:08X}",
	...
	Result = Device->CreateDepthStencilView(
	    DepthStencilBuffer.Get(), &DepthViewDesc, DepthStencilView.ReleaseAndGetAddressOf());
	panicf(SUCCEEDED(Result), "ID3D11Device::CreateDepthStencilView 실패. HRESULT=0x{:08X}",
	       static_cast<uint32>(Result));
}
