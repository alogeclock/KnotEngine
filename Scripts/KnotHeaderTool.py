...
class HeaderTool:
    ...
    def generate(self, output: Path, module: str, owned_headers: set[Path]) -> list[Path]:
        ...
        prefix += "".join(f'#include "{path.as_posix()}"\n' for path in sorted({item.path for item in self.types.values()}))
        ...
        for item in self.types.values():
            if item.kind == "enum":
                ...
                prefix += f"template <> {export}UEnum* StaticEnum<{item.name}>();\n"
        ...
        content += f"\nvoid {register_function}(FReflectionRegistry& Registry)\n{{\n"
        content += "".join(f"\tTReflectionAccess<{item.name}>::Register(Registry);\n" for item in ordered)
        content += "\n"
