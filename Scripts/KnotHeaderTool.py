...
class HeaderTool:
    ...
    def property_expression(self, value_type, name: str, owner: str, offset: str, flags: str, cursor, allow_const: bool = False) -> str:
        ...
        if declared_name in {"TObjectPtr", "TSoftObjectPtr"}:
            ...
            self.referenced_types.add(target_name)
        ...
        if reflected and reflected.kind in {"struct", "enum"}:
            ...
            self.includes.add(prop)
            self.referenced_types.add(declared_name)
    ...
    def generate_type(self, item: ReflectedType) -> str:
        ...
        if item.kind == "class" and self.constructible.get(name):
            lines += ["\tstatic UObject* CreateObject(UClass& Class, UObject* Outer, FName Name, UObject* Template, EObjectFlags Flags)", "\t{",
                      f"\t\tpanic(&Class == {name}::StaticClass());", "\t\tFObjectInitializer& Initializer = FObjectInitializer::Get();",
                      "\t\tcheck(&Initializer.GetClass() == &Class && Initializer.GetOuter() == Outer && Initializer.GetTemplate() == Template &&",
                      "\t\t      Initializer.GetName() == Name && Initializer.GetFlags() == Flags);", f"\t\treturn new {name}();", "\t}"]
        reset_target = f"{name}::StaticClassPrivate" if item.kind == "class" else f"{name}::StaticStructPrivate" if item.kind == "struct" else "StaticEnumPrivate"
        lines += [f"\tstatic void Reset() {{ {reset_target} = nullptr; }}", "};", ""]
        if item.kind != "enum":
    ...
    def generate(self, output: Path, module: str, owned_headers: set[Path]) -> list[Path]:
        ordered = []
        ...
        # Dependency declarations remain available for validation, but their definitions
        # and static schema storage must be emitted only by the owning DLL.
        ordered = [item for item in ordered if item.path in owned_headers]

        types_by_header: dict[Path, list[ReflectedType]] = {header: [] for header in owned_headers}
        for item in ordered:
            types_by_header.setdefault(item.path, []).append(item)

        outputs = []
        for header in sorted(types_by_header):
            items = types_by_header[header]
            self.includes = set()
            self.referenced_types = {item.parent for item in items if item.parent}
            content = "\n".join(self.generate_type(item) for item in items)

            include_paths = {header}
            for name in self.referenced_types:
                reflected = self.types.get(name)
                if reflected:
                    include_paths.add(reflected.path)

            prefix = "// Generated by KnotHeaderTool. Do not edit.\n"
            prefix += "".join(f'#include "{path.as_posix()}"\n' for path in sorted(include_paths))
            prefix += '#include "Object/Reflection/Class.h"\n#include "Object/Reflection/ReflectionRegistry.h"\n'
            if "@Function" in self.includes:
                prefix += '#include "Object/Reflection/Function.h"\n'
            prefix += "".join(f'#include "Object/Property/{name}.h"\n' for name in sorted(self.includes) if not name.startswith("@"))
            prefix += "#include <cstddef>\n\n"

            enum_names = {item.name for item in items if item.kind == "enum"}
            enum_names.update(name for name in self.referenced_types if name in self.types and self.types[name].kind == "enum")
            for name in sorted(enum_names):
                item = self.types[name]
                export = "ENGINE_API " if module == "Engine" or item.path not in owned_headers else ""
                prefix += f"template <> {export}UEnum* StaticEnum<{name}>();\n"
            if enum_names:
                prefix += "\n"

            for item in items:
                content += f"\nvoid KnotRegister_{item.symbol}(FReflectionRegistry& Registry)\n{{\n"
                content += f"\tTReflectionAccess<{item.name}>::Register(Registry);\n}}\n"
                content += f"\nvoid KnotPopulate_{item.symbol}()\n{{\n"
                content += f"\tTReflectionAccess<{item.name}>::Populate();\n}}\n"
                content += f"\nvoid KnotReset_{item.symbol}()\n{{\n"
                content += f"\tTReflectionAccess<{item.name}>::Reset();\n}}\n"

            try:
                relative = header.relative_to(self.source / module)
            except ValueError as error:
                raise RuntimeError(f"Reflected header is outside the {module} source directory: {header}") from error
            generated_path = output / relative.with_suffix(".gen.cpp")
            write_changed(generated_path, prefix + content)
            outputs.append(generated_path)

        for path, markers in self.markers.items():
            if path not in owned_headers:
                continue
        ...
        prefix = "// Generated by KnotHeaderTool. Do not edit.\n"
        prefix += '#include "Object/Reflection/ReflectionRegistry.h"\n'
        prefix += '#include "Runtime/EditorEngine.h"\n\n' if module == "Editor" else '#include "Object/Reflection/EngineReflection.h"\n\n'
        for item in ordered:
            prefix += f"void KnotRegister_{item.symbol}(FReflectionRegistry& Registry);\n"
            prefix += f"void KnotPopulate_{item.symbol}();\n"
            prefix += f"void KnotReset_{item.symbol}();\n"
        prefix += "\n"

        register_function = "UEditorEngine::RegisterTypes" if module == "Editor" else "RegisterEngineTypes"
        reset_function = "UEditorEngine::ResetTypes" if module == "Editor" else "ResetEngineTypes"
        content = f"void {register_function}(FReflectionRegistry& Registry)\n{{\n"
        content += "".join(f"\tKnotRegister_{item.symbol}(Registry);\n" for item in ordered)
        content += "\n"
        content += "".join(f"\tKnotPopulate_{item.symbol}();\n" for item in ordered)
        content += f"}}\n\nvoid {reset_function}()\n{{\n"
        for item in reversed(ordered):
            content += f"\tKnotReset_{item.symbol}();\n"
        content += "}\n"
        generated_path = output / "Registration.gen.cpp"
        write_changed(generated_path, prefix + content)
        outputs.append(generated_path)
        return outputs
...
def main() -> int:
    ...
    parser.add_argument("--module", choices=("Engine", "Editor"), required=True)
    ...
    owned_headers = set(headers)
    if options.dependencies:
        headers += [Path(line).resolve() for line in options.dependencies.read_text(encoding="utf-8-sig").splitlines() if line]
    ...
    if options.dependencies:
        inputs.append(options.dependencies)
    ...
    outputs = tool.generate(output, options.module, owned_headers)
    previous_outputs = [Path(path) for path in state.get("outputs", [])] if state else []
    for stale in set(previous_outputs) - set(outputs):
        try:
            stale.resolve().relative_to(output.resolve())
        except ValueError:
            continue
    ...
    if options.stamp:
        options.stamp.parent.mkdir(parents=True, exist_ok=True)
        options.stamp.touch()
    owned_count = sum(item.path in owned_headers for item in tool.types.values())
    print(f"KnotHeaderTool: generated {owned_count} {options.module} types; validated {len(tool.types) - owned_count} dependency types.")
    return 0
